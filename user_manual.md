...
3. You can run P2Ping with the python command `python main.py` in a terminal while in the src directory for P2Ping. You can also find main.py through your file exployer by going to the downloaded P2Ping repository and opening the src folder. Once you have found python.py you can double-click the file or right click it and selecting the option to run it. The P2Ping aplication should then run and the window should pop up. For ease of access, you can create a shortcut to the file by right clicking on it, selecting "Show more options", then selecting "Send to" and then selecting desktop which will create a desktop shortcut for the file.
...
5. A chat should open up and you can send messages and files to each other directly.
6. Simply exit the program when you are finished messaging each other. You can also end the connection with the red "End Connection" button and start a new one with a different peer if desired. 

**NOTICE:** Encrypting files and messages is still a work in progess. Currently messages are not saved. Any messages will be deleted when the application is terminated or when you hit the "clear messages" button. 
