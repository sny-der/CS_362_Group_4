...
Triggers: Sender has pressed the send file button in the chat window. 

Preconditions: 
- Both sender and receiver have the application installed and running.
- Both sender and the receiver have connected to each other.
- The file to be sent exists on the Sender’s device and is accessible.
- The receiver is allowing file receiving at this time.
  
Postconditions:
- The file is transferred from Sender to Receiver.
- The Receiver receives and can access the file on their device.
- No intermediate server or third party ever accesses the file content.
- If the Receiver was offline, the file is queued on the Sender’s device and delivered automatically once the Receiver comes online. (stretch goal - WIP)

List of Steps:
1. The Sender selects the “Send File” option.
2. The Sender chooses the file from their device
3. The app uses the established P2P connection.
4. The app sends the chunks + header to Receiver
5. The Receiver’s app receives the chunks, and re-combines them
6. The Receiver’s app saves the file locally
7. The Receiver’s app sends a delivery acknowledgement to Sender
8. The Sender’s app displays “Sent File: (file name)”


Extensions / Variations of the Success Scenario
- Variation 1: Receiver Offline at Send Time (stretch goal - WIP)
   - App detects Receiver is offline
   - The app stores it locally in an encrypted queue.
   - Background process periodically checks Receiver status.
   - When the receiver comes online, starts sending process
...
* **Success Metric:** A test is considered "Passed" only if the SHA-256 hash of the received file perfectly matches the source file hash after being processed through the encryption and chunking pipeline.
  ...
  ### Abram Gallup:
  - Working on the project as a group in person or a call with the ability to share screens allowed me to ask clarifying questions from the group and get an immediate answer. It helped keep us on the same page and to stay focused on the project. We should have called and met more often, I think we could have overcome challenges better and gotten even more of the project done.
  - I wish I had entered this project with more prior experience in P2P networking and socket programming or atleast chose a project in an area I had more experience in. Because this was a relatively new area for me, a lot of the early architecture was a "learn-as-you-go" process. If I had a stronger foundation from the start, I think I could have made smarter decisions regarding our initial protocol design, which would have saved us time refactoring code later in the execution phase.
  - If I had a stronger foundation from the start, we could have divided the project labor more evenly. For example, putting only one person in charge of the peer connection and data transmission created a bottleneck we weren't expecting. There should have been at least two people researching and implementing this code together on a weekly or even daily basis. What also complicated this was the fact that not everyone in our group used Windows. Implementing this project across platforms was difficult and should have been kept in mind when we were deciding how we wanted to implement the project—specifically regarding the programming language, choosing a web-based vs. desktop UI, and using UDP vs. TCP.
  ### Charles Weber:
  - The biggest thing I learned is not only the importance of communication, but also communicating the correct information. We did this pretty well but I am sure there is room for improvement.
  - Something I realized was that our inexperience in this field showed alot. We struggled with delegating tasks because we didn't know how the project would work once complete. We transitioned from a linked list of packets to a bitmap for file transferring. Some of us ended up doing alot of one type of work because we couln't ask our teamates to help us. This was because we were not sure what we would need of them. If we were a more experienced team we would be able to plan out the implementation details, thus allowing delegation.
  - This project turned out well and I learned alot. Not just about the internet but the nuances of software engineering. One of the design decisions that I will definately repeat in the future is having function consisntency. The second version of our code had a special python file that would interact with the C file. The rest of the code (wich was in python) would call functions from this special python file to interact with the C code. This was done through a loopback IPv6 packet transfer system. The packets woul exchange information between python and C. There was a communication standard that both files respected. When it came to making version three, all of the changes were under the hood in the C file. The C file had to be modified. The python file, and by extention the rest of the code, did not have to be modified because it was built in a standardised way. This standardization saved us alot of time and I am greatful I learned such an impactful skill.
  ### Arjun Bhave:
- Something important that I learned while working on P2Ping was to split large features into small steps. At the beginning of the project while developing some of the features like networking, encryption and GUI, seemed large. Splitting the development and debugging process into small steps made things easier to manage.
- Regular testing throughout development was essential. Due to the nature of the program pinging between peers, small bugs/errors within networking and message handling could cause the entire program to fail. Testing more often caught issues earlier and avoided bigger issues down the road.
- Teamwork and keeping the lines of communication open helped us work more efficiently. Talking through design decisions/troubleshooting allowed us to come up with better solutions and confirm everyone was on the same page.
